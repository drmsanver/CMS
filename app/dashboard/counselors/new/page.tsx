...
export default function NewCounselorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(`Counselor ${formData.name} was successfully registered.`);
      setFormData({ name: "", email: "", passwordRaw: "" });
      router.refresh();
      setTimeout(() => router.push('/dashboard/counselors'), 1500);
    } catch (err: any) {
      setError(err.message || "Failed to register counselor.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="glass-panel" style={{ padding: '2.5rem' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} style={{ padding: '0.875rem', borderRadius: '8px', border: '1px solid var(--border-strong)', background: 'var(--bg-main)', color: 'var(--text-primary)' }} placeholder="Jane Doe" />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            <label style={{ fontSize: '0.875rem', fontWeight: 500 }}>Email Address</label>
            <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} style={{ padding: '0.875rem', borderRadius: '8px', border: '1px solid var(--border-strong)', background: 'var(--bg-main)', color: 'var(--text-primary)' }} placeholder="jane.doe@school.com" />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>The user will log in with this string.</span>
          </div>
